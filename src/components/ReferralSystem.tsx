...
export default function ReferralSystem() {
  ...
  const copyReferralCode = () => {
    navigator.clipboard.writeText(
      `Join VANHSYA with my referral code: ${referralCode} and get ₹5,000 bonus! https://vanhsya.com/ref/${referralCode}`
    );
    setCopied(true);
  ...
  const shareReferral = (platform: string) => {
    ...
    switch (platform) {
      ...
      case 'facebook':
        window.open(
          `https://facebook.com/sharer/sharer.php?u=${url}&quote=${encodeURIComponent(text)}`
        );
        break;
      case 'twitter':
        window.open(
  ...
  const currentTierIndex = referralTiers.findIndex(
    tier => tier.name === stats.currentTier
  );
        ...
        >
          <GlassServiceCard className='bg-gradient-to-r from-purple-600/90 to-pink-600/90 backdrop-blur-xl border-white/30'>
            ...
            <div className='space-y-2'>
              ...
              <div className='text-white text-sm'>
                Referrals:{' '}
                <span className='font-semibold'>{stats.totalReferrals}/10</span>
              </div>
                ...
                <div className='mb-8 p-6 bg-gradient-to-r from-purple-600/20 to-pink-600/20 rounded-xl border border-purple-500/30'>
                  <div className='flex items-center justify-between mb-3'>
                    ...
                    <span className='text-white/70'>
                      {stats.totalReferrals}/{nextTier.referrals}
                    </span>
                  </div>
                  <div className='w-full bg-white/20 rounded-full h-3 mb-2'>
                    <motion.div
                      ...
                      initial={{ width: 0 }}
                      animate={{
                        width: `${(stats.totalReferrals / nextTier.referrals) * 100}%`,
                      }}
                      transition={{ duration: 1.2 }}
                    />
                  </div>
                  <p className='text-white/80 text-sm'>
                    Just {nextTier.referrals - stats.totalReferrals} more
                    referrals to unlock {nextTier.reward}!
                  </p>
                </div>
              )}
                  ...
                  return (
                    ...
                    >
                      <div className='text-center'>
                        <div
                          className={`w-12 h-12 rounded-full bg-gradient-to-r ${tier.color} flex items-center justify-center mx-auto mb-3`}
                        >
                        ...
                        <div className='text-lg font-bold text-accent-400 mb-3'>
                          {tier.reward}
                        </div>
              ...
              <div className='bg-gradient-to-r from-slate-800 to-slate-700 rounded-xl p-6'>
                ...
                <div className='grid md:grid-cols-3 gap-6'>
                  ...
                  <div className='text-center'>
                    ...
                    <p className='text-white/70 text-sm'>
                      Your friends join VANHSYA using your referral code
                    </p>
                  </div>
                  <div className='text-center'>
                    <div className='w-10 h-10 bg-primary-600 rounded-full flex items-center justify-center mx-auto mb-2'>
                    ...
                    <p className='text-white/70 text-sm'>
                      Get cashback and climb tiers for bigger rewards
                    </p>
                  </div>
                </div>
              </div>
