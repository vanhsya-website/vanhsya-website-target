...
const GlobalMigrationHeatmap: React.FC = () => {
  ...
  const [viewMode, setViewMode] = useState<'heatmap' | 'flows' | 'trends'>('heatmap');
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setLiveStats(prev => ({
        totalApplications: prev.totalApplications + Math.floor(Math.random() * 10),
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className={`text-xl ${VanhsyaTheme.text.secondary} max-w-4xl mx-auto mb-8`}>
            Real-time migration trends, application flows, and success analytics across 150+ countries. 
            Powered by VANHSYA's AI intelligence network.
          </p>
        </motion.div>
        ...
        >
          {[
            { label: 'Total Applications', value: liveStats.totalApplications.toLocaleString(), icon: Users, color: 'from-violet-500 to-purple-600' },
            { label: 'Approved Today', value: liveStats.approvedToday.toLocaleString(), icon: Award, color: 'from-green-500 to-emerald-600' },
            { label: 'Processing', value: liveStats.processing.toLocaleString(), icon: RefreshCw, color: 'from-blue-500 to-cyan-600' },
            { label: 'Avg Wait Time', value: `${liveStats.avgWaitTime.toFixed(1)} months`, icon: Calendar, color: 'from-orange-500 to-red-600' }
          ].map((stat, index) => (
            <motion.div
              key={index}
        ...
        >
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <h3 className={`text-lg font-bold ${VanhsyaTheme.text.primary}`}>View Mode:</h3>
              <div className="flex gap-2">
                {[
                  { id: 'heatmap' as const, label: 'Heatmap', icon: BarChart3 },
                  { id: 'flows' as const, label: 'Migration Flows', icon: TrendingUp },
            ...
            <div className="flex items-center gap-4">
              <h3 className={`text-lg font-bold ${VanhsyaTheme.text.primary}`}>Timeframe:</h3>
              <div className="flex gap-2">
                {[
          ...
          <div className="lg:col-span-2">
            ...
            >
              ...
              <div className="relative w-full h-full bg-gradient-to-br from-slate-800/50 to-indigo-900/50 rounded-xl overflow-hidden">
                  ...
                  >
                    <motion.div
                      className={`w-4 h-4 rounded-full bg-gradient-to-r ${
                        country.trend === 'up' ? 'from-green-500 to-emerald-600' :
                        country.trend === 'down' ? 'from-red-500 to-red-600' :
                    ...
                    />
                    <div className={`absolute -top-8 left-1/2 transform -translate-x-1/2 text-xs ${VanhsyaTheme.text.primary} font-medium whitespace-nowrap`}>
                      {country.name}
                    </div>
                  </motion.div>
                ))}
                ...
                {viewMode === 'flows' && migrationFlows.map((flow, index) => (
                  <motion.div
                    key={index}
              ...
              {selectedCountry ? (
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <span className={VanhsyaTheme.text.secondary}>Total Migrants</span>
                  ...
                  <div className="flex items-center justify-between">
                    <span className={VanhsyaTheme.text.secondary}>Growth Rate</span>
                    <span className={`font-bold ${getTrendColor(selectedCountry.trend)}`}>
                      {getTrendIcon(selectedCountry.trend)} {Math.abs(selectedCountry.growthRate)}%
                    </span>
                  </div>
                  
                  <div className="flex items-center justify-between">
                    <span className={VanhsyaTheme.text.secondary}>Success Rate</span>
                    <span className={`font-bold ${VanhsyaTheme.text.primary}`}>
                  ...
                  <div className="flex items-center justify-between">
                    <span className={VanhsyaTheme.text.secondary}>Avg Processing</span>
                    <span className={`font-bold ${VanhsyaTheme.text.primary}`}>
                  ...
                  <div>
                    <span className={`${VanhsyaTheme.text.secondary} block mb-2`}>Popular Categories</span>
                    <div className="flex flex-wrap gap-2">
                      {selectedCountry.popularCategories.map((category, index) => (
                        <span
                          key={index}
                ...
                <p className={VanhsyaTheme.text.secondary}>
                  Click on a country hotspot to view detailed migration statistics and trends.
                </p>
              )}
            </motion.div>
            ...
            >
              ...
              <div className="space-y-3">
                {migrationFlows.slice(0, 5).map((flow, index) => (
                  ...
                  >
                    <div>
                      <div className={`font-medium ${VanhsyaTheme.text.primary}`}>
                        {flow.from} → {flow.to}
                      </div>
                    ...
                    <div className={`text-sm font-medium ${getTrendColor(flow.trend)}`}>
