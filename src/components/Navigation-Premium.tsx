...
const navigationItems: NavigationItem[] = [  {    label: 'AI Tools',    icon: SparklesIcon,    badge: 'NEW',    description: 'Cutting-edge AI-powered immigration tools',    children: [      {         label: 'VanHologram Consultant',         href: '/ai-tools/vanhologram',        description: 'AI-powered personal immigration consultant',        featured: true      },      {         label: 'Document Analyzer',         href: '/ai-tools/document-wizard',        description: 'Intelligent document validation and processing'      },      {         label: 'Immigration Calculator',         href: '/ai-tools/immigration-calculator',        description: 'Calculate your immigration score instantly'      },      {         label: 'Country Comparison',         href: '/ai-tools/country-comparison',        description: 'AI-driven country matching system'      },      {         label: 'Eligibility Checker',         href: '/ai-tools/eligibility-bot',        description: 'Smart eligibility assessment tool'      },      {         label: 'ImmigrationDNA',         href: '/ai-tools/immigration-dna',        description: 'Advanced profile matching algorithm',        badge: 'BETA'      },    ]  },  {    label: 'Countries',    href: '/countries',    icon: GlobeAmericasIcon,    description: 'Explore immigration destinations worldwide',    children: [      { label: 'Canada', href: '/countries/canada' },      { label: 'Australia', href: '/countries/australia' },      { label: 'New Zealand', href: '/countries/new-zealand' },      { label: 'Germany', href: '/countries/germany' },      { label: 'United Kingdom', href: '/countries/uk' },      { label: 'Netherlands', href: '/countries/netherlands' },      { label: 'All Countries', href: '/countries', featured: true },    ]  },  {    label: 'Services',    href: '/services',    icon: WrenchScrewdriverIcon,    description: 'Comprehensive immigration services',    children: [      {         label: 'Skilled Migration',         href: '/services/skilled-migration',        description: 'Professional immigration pathways'      },      {         label: 'Family Sponsorship',         href: '/services/family-sponsorship',        description: 'Reunite with your loved ones'      },      {         label: 'Student Visas',         href: '/services/student-visas',        description: 'Education-based immigration'      },      {         label: 'Business Immigration',         href: '/services/business-immigration',        description: 'Investment and entrepreneur visas'      },      {         label: 'Express Entry',         href: '/services/express-entry',        description: 'Fast-track immigration programs'      },    ]  },  {    label: 'Success Stories',    href: '/success-stories',    icon: TrophyIcon,    description: 'Real client immigration journeys'  },  {    label: 'Resources',    icon: BookOpenIcon,    description: 'Guides, tools, and immigration insights',    children: [      {         label: 'Immigration Guides',         href: '/resources/guides',        description: 'Step-by-step immigration guides'      },      {         label: 'Document Checklists',         href: '/resources/documents',        description: 'Required documents by country'      },      {         label: 'FAQ',         href: '/faq',        description: 'Frequently asked questions'      },      {         label: 'Blog',         href: '/blog',        description: 'Latest immigration news and insights'      },      {         
        label: 'Webinars',         
...
export default function NavigationEnhanced() {
  ...
  );  const SearchModal = () => (    <AnimatePresence>      {searchOpen && (        <motion.div          initial={{ opacity: 0 }}          animate={{ opacity: 1 }}          exit={{ opacity: 0 }}          className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm"          onClick={() => setSearchOpen(false)}        >          <motion.div            initial={{ opacity: 0, y: -50 }}            animate={{ opacity: 1, y: 0 }}            exit={{ opacity: 0, y: -50 }}            className="max-w-2xl mx-auto mt-20 overflow-hidden bg-white border border-gray-200 shadow-xl rounded-xl"            onClick={(e) => e.stopPropagation()}          >            <div className="p-6">              <div className="flex items-center gap-3 mb-4">                <MagnifyingGlassIcon className="w-6 h-6 text-gray-400" />                <input                  type="text"                  placeholder="Search countries, services, tools..."                  value={searchQuery}                  onChange={(e) => setSearchQuery(e.target.value)}                  className="flex-1 text-lg font-medium text-gray-900 placeholder-gray-400 border-none outline-none"                  autoFocus                />
              </div>
              
              {/* Quick Suggestions */}
              <div className="space-y-3">
                <h3 className="text-sm font-medium tracking-wide text-gray-500 uppercase">Quick Access</h3>
                <div className="grid grid-cols-2 gap-2">
                  {[
                    { label: 'Immigration Calculator', href: '/ai-tools/immigration-calculator' },
                    { label: 'Canada Immigration', href: '/countries/canada' },
                    { label: 'Document Analyzer', href: '/ai-tools/document-wizard' },
    ...
    <>
      ...
      <motion.div 
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          hideStatsBar ? 'transform -translate-y-full opacity-0' : 'transform translate-y-0 opacity-100'
        }`}
      >
          ...
          <div className="container px-4 py-2 mx-auto">
            ...
            >
              ...
              >
                ...
                <span className="hidden opacity-90 sm:inline">Visas Approved</span>
              </motion.div>
              ...
              >
                ...
                <span className="font-semibold text-gold-300">Leading the New Migration Era</span>
                <motion.div
                  animate={{ rotate: 360 }}
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <Link href="/" className="flex items-center gap-3 group">
              ...
              <div>
                ...
                <div className="text-xs font-medium text-purple-600">AI-Powered Immigration</div>
              </div>
            </Link>
              ...
              {navigationItems.map((item, index) => (
                ...
                >
                  {item.href ? (
                    ...
                    <button
                      className="flex items-center gap-2 px-4 py-3 font-semibold text-gray-700 transition-all duration-200 rounded-xl hover:text-purple-700 hover:bg-gradient-to-r hover:from-purple-50 hover:to-blue-50"
                    >
                      {item.icon && <item.icon className="w-5 h-5" />}
                      <span className="font-semibold">{item.label}</span>
                      {item.badge && (
