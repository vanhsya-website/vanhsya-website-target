...
export default function ScamShieldVerifier() {
  ...
  const analyzeDocument = async () => {
    ...
    const mockResult: ScamDetectionResult = {
      ...
      recommendations: [],
      analysis: {
        documentType: uploadedFile.name.includes('visa')
          ? 'Visa Document'
          : uploadedFile.name.includes('permit')
            ? 'Work Permit'
            : uploadedFile.name.includes('job')
              ? 'Job Offer Letter'
    ...
    const failedChecks = mockResult.analysis.validityChecks.filter(
      check => check.status === 'fail'
    );
    const warningChecks = mockResult.analysis.validityChecks.filter(
  ...
  const getStatusIcon = (status: 'pass' | 'fail' | 'warning') => {
    switch (status) {
      case 'pass':
        return <FaCheck className='w-4 h-4 text-green-400' />;
      case 'fail':
              ...
              {result && (
                <div className='space-y-6'>
                  ...
                  <div className='text-center p-6 rounded-xl bg-gradient-to-r from-slate-800 to-slate-700'>
                    <div
                      className={`text-6xl font-bold mb-2 ${result.isAuthentic ? 'text-green-400' : 'text-red-400'}`}
                    >
                      {result.isAuthentic ? '✓' : '✗'}
                    </div>
                    ...
                    <p
                      className={`text-lg ${getConfidenceColor(result.confidence)}`}
                    >
                    ...
                    <div>
                      ...
                      <div className='space-y-3'>
                        ...
                        <div className='flex justify-between'>
                          ...
                          <span className='text-white'>
                            {result.analysis.issuer}
                          </span>
                        </div>
                      </div>

                      <h5 className='text-md font-medium text-white mt-6 mb-3'>
                        Security Checks:
                      </h5>
                  ...
                  <div className='flex gap-4 pt-6 border-t border-white/20'>
                    ...
                    <button className='flex-1 bg-gradient-to-r from-primary-600 to-accent-500 text-white py-3 rounded-lg hover:opacity-90 transition-opacity flex items-center justify-center gap-2'>
