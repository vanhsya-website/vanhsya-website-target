...
import { 
  MapPin, Clock, DollarSign, Users, FileText, CheckCircle, Star, ArrowRight, 
  Phone, Globe, TrendingUp, Award, Briefcase, GraduationCap, Heart, 
  Plane, Building, AlertCircle, Info, ExternalLink
} from 'lucide-react';
...
export default function EnhancedAustraliaPage({ countryData }: EnhancedCountryPageProps) {
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig = {
      ...
      'Coming Soon': { bg: 'bg-yellow-100', text: 'text-yellow-800', icon: Clock },
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            >
              <div className="flex items-center gap-4 mb-6">
                ...
                <h1 className="text-5xl lg:text-7xl font-bold">
                  <span className="text-white drop-shadow-lg">Immigrate to</span>
                  <span className="block text-white drop-shadow-lg">{countryData.name}</span>
                </h1>
              </div>
            ...
            >
              {countryData.stats.slice(0, 4).map((stat, index) => (
                <div key={index} className="bg-white/90 backdrop-blur-sm rounded-2xl p-6 text-center shadow-xl">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <h2 className="text-4xl lg:text-5xl font-bold text-gray-800 mb-6">
              Complete <span className="text-gradient-blue">Migration Programs</span>
            </h2>
            <p className="text-xl text-gray-600 max-w-3xl mx-auto">
              Comprehensive guide to all Australian visa categories including skilled, temporary, family, business, and visitor options
            </p>
          </motion.div>
          ...
          {countryData.skillCategories && (
            <div className="flex flex-wrap justify-center gap-4 mb-12">
              {Object.entries(countryData.skillCategories).map(([key, category]: [string, any]) => {
          ...
          {countryData.skillCategories && (
            ...
            >
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {countryData.skillCategories[selectedCategory]?.programs?.map((program: any, index: number) => (
                  <motion.div
                    key={index}
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                  ...
                  >
                    ...
                    <h4 className="text-xl font-bold text-gray-800 mb-3">{program.name}</h4>
                    ...
                    <div className="space-y-2 mb-4">
                      ...
                      <div className="flex justify-between text-sm">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span className="font-semibold">{program.popularity}%</span>
                        </div>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-500">Success Rate:</span>
                        <span className="font-semibold">{program.success}</span>
                      </div>
                      {program.basePrice > 0 && (
                        <div className="flex justify-between text-sm">
                          <span className="text-gray-500">Government Fee:</span>
                          <span className="font-semibold text-blue-600">AUD {program.basePrice}</span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="mb-4">
                      ...
                      <p className="text-sm font-medium text-gray-700">{program.eligibility}</p>
                    </div>
                    ...
                    >
                      {program.status === 'Active' ? 'Apply Now' : program.status}
                      {program.status === 'Active' && <ExternalLink className="w-4 h-4" />}
                    </motion.button>
                  </motion.div>
                ))}
              </div>
              ...
              <div className="grid md:grid-cols-3 gap-6">
                {countryData.latestUpdates.map((update: any, index: number) => (
                  <div key={index} className="bg-white rounded-xl p-6 shadow-md">
                    ...
                    <div className="text-xs text-green-600 font-medium">{update.impact}</div>
                  </div>
                ))}
              </div>
            </motion.div>
        ...
        <section className="py-20 bg-gray-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            >
              ...
              <p className="text-xl text-gray-600 max-w-3xl mx-auto">
                Each Australian state offers unique opportunities and nomination programs
              </p>
            </motion.div>
              ...
              <div className="space-y-4">
                {countryData.provinces.map((province, index) => (
                  ...
                  >
                    ...
                    <p className={`text-sm ${selectedProvince === index ? 'text-blue-100' : 'text-gray-600'}`}>
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div className="bg-blue-50 rounded-lg p-4">
                    <div className="text-blue-600 font-semibold">Population</div>
                ...
                {countryData.provinces[selectedProvince].minPoints && (
                  <div className="grid grid-cols-2 gap-4 mb-6">
                    <div className="bg-yellow-50 rounded-lg p-4">
                      <div className="text-yellow-600 font-semibold">Min Points</div>
                      <div className="text-2xl font-bold text-gray-800">
                        {countryData.provinces[selectedProvince].minPoints}
                      </div>
                    </div>
                    <div className="bg-purple-50 rounded-lg p-4">
                      <div className="text-purple-600 font-semibold">Processing</div>
                ...
                <div className="mb-6">
                  <h4 className="font-bold text-gray-800 mb-3">Available Programs:</h4>
                  <div className="grid grid-cols-1 gap-2">
                    {countryData.provinces[selectedProvince].programs.map((program, idx) => (
                ...
                <div className="mb-6">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {countryData.provinces[selectedProvince].highlights.map((highlight, idx) => (
                      <div key={idx} className="flex items-center gap-2 text-sm text-gray-600">
                ...
                {countryData.provinces[selectedProvince].industries && (
                  <div>
                    <h4 className="font-bold text-gray-800 mb-3">Key Industries:</h4>
                    <div className="flex flex-wrap gap-2">
                      {countryData.provinces[selectedProvince].industries?.map((industry, idx) => (
        ...
        <section className="py-20 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            >
              ...
              <p className="text-xl text-gray-600 max-w-3xl mx-auto">
                Comprehensive breakdown of living expenses across major Australian cities
              </p>
            </motion.div>

            <div className="grid lg:grid-cols-2 gap-8">
              <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl p-8">
                <h3 className="text-2xl font-bold text-gray-800 mb-6">Housing Costs (Monthly Rent)</h3>
                <div className="space-y-4">
                  {Object.entries(countryData.costOfLiving.housing).map(([city, cost]) => (
                    <div key={city} className="flex justify-between items-center p-4 bg-white rounded-lg">
                      <span className="font-medium text-gray-800 capitalize">{city}</span>
                      <span className="font-bold text-blue-600">{String(cost)}</span>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="bg-white rounded-xl p-6 shadow-lg border border-gray-100">
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold">{countryData.costOfLiving.utilities}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Groceries (Family of 4)</span>
                      <span className="font-semibold">{countryData.costOfLiving.groceries}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Public Transport</span>
                      <span className="font-semibold">{countryData.costOfLiving.transport}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Restaurant Meal</span>
                      <span className="font-semibold">{countryData.costOfLiving.dining}</span>
                    </div>
                  </div>
                </div>

                <div className="bg-green-50 rounded-xl p-6">
                  <h4 className="font-bold text-green-800 mb-2">💡 Cost-Saving Tips</h4>
