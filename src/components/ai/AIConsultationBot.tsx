...
interface ConversationContext {
  ...
  conversationStage: 'greeting' | 'assessment' | 'planning' | 'documents' | 'application' | 'followup';
  detectedIntents: string[];
}

const quickStartTopics = [
  { icon: GlobeAmericasIcon, text: 'Which country is best for me?', category: 'assessment' },
  { icon: DocumentTextIcon, text: 'What documents do I need?', category: 'documents' },
  { icon: ClockIcon, text: 'How long will the process take?', category: 'timeline' },
  { icon: AcademicCapIcon, text: 'Education credential assessment', category: 'credentials' },
  { icon: BriefcaseIcon, text: 'Job market and opportunities', category: 'employment' },
  { icon: CheckCircleIcon, text: 'Check my eligibility', category: 'eligibility' },
];

const predefinedResponses = {
  greeting: {
    triggers: ['hello', 'hi', 'hey', 'start', 'help'],
    response: "Hello! I'm VanHologram, your AI immigration consultant. I'm here to guide you through every step of your immigration journey. What would you like to explore today?",
    suggestions: ['Assess my eligibility', 'Compare countries', 'Document requirements', 'Application timeline']
  },
  assessment: {
    triggers: ['eligibility', 'qualify', 'assess', 'evaluate', 'score'],
    response: "I'd be happy to assess your immigration prospects! Let me ask you a few quick questions to provide personalized guidance. What's your highest level of education?",
    suggestions: ['High School', 'Bachelor\'s Degree', 'Master\'s Degree', 'PhD/Doctorate']
  },
  countries: {
    triggers: ['country', 'destination', 'where', 'best', 'recommend'],
    response: "Great question! The best destination depends on your profile, goals, and preferences. Based on current trends, here are some top options:",
    suggestions: ['Canada 🇨🇦', 'Australia 🇦🇺', 'New Zealand 🇳🇿', 'Germany 🇩🇪']
  },
  documents: {
    triggers: ['document', 'paper', 'certificate', 'requirement'],
    response: "Document preparation is crucial for a successful application. Here's what you'll typically need:",
    suggestions: ['Educational documents', 'Language test results', 'Work experience letters', 'Financial statements']
  },
  timeline: {
    triggers: ['time', 'long', 'duration', 'process', 'wait'],
    response: "Immigration timelines vary by country and visa type. Here's a general overview:",
    suggestions: ['Express Entry (6-8 months)', 'Skilled Migration (8-12 months)', 'Family Sponsorship (12-24 months)']
  }
};
...
export default function AIConsultationBot() {
  ...
  useEffect(() => {
    if (isOpen && messages.length === 0) {
      ...
      const welcomeMessage: Message = {
        ...
        type: 'bot',
        content: "👋 Hello! I'm VanHologram, your AI-powered immigration consultant. I'm here to provide personalized guidance for your immigration journey.",
        timestamp: new Date(),
        metadata: {
          suggestions: ['Get started with assessment', 'Compare immigration destinations', 'Check document requirements', 'Ask about timelines'],
          quickActions: [
  ...
  const detectIntent = (message: string): string[] => {
    ...
    if (lowercaseMessage.includes('family') || lowercaseMessage.includes('spouse')) {
      intents.push('family');
    }
    if (lowercaseMessage.includes('cost') || lowercaseMessage.includes('fee') || lowercaseMessage.includes('price')) {
      intents.push('cost');
    }
    if (lowercaseMessage.includes('job') || lowercaseMessage.includes('work') || lowercaseMessage.includes('employment')) {
  ...
  const generateResponse = (userMessage: string, detectedIntents: string[]): Message => {
    ...
    if (detectedIntents.includes('assessment') || detectedIntents.includes('eligibility')) {
      return {
        id: Date.now().toString(),
        type: 'bot',
        content: "I'll help you assess your immigration eligibility! Based on factors like age, education, work experience, and language skills, I can provide a comprehensive evaluation. Would you like to start with a quick assessment?",
        timestamp: new Date(),
        metadata: {
          suggestions: ['Start quick assessment', 'Use detailed calculator', 'Compare my options', 'Get expert review'],
          links: [
            { text: 'Immigration Calculator', url: '/ai-tools/immigration-calculator' },
    ...
    if (detectedIntents.includes('countries')) {
      return {
        id: Date.now().toString(),
        type: 'bot',
        content: "Let me help you find the perfect destination! Each country has unique advantages:\n\n🇨🇦 **Canada**: Express Entry system, high success rates\n🇦🇺 **Australia**: Points-based system, excellent quality of life\n🇳🇿 **New Zealand**: Beautiful landscapes, work-life balance\n🇩🇪 **Germany**: Strong economy, EU access\n\nWhat factors are most important to you?",
        timestamp: new Date(),
        metadata: {
          suggestions: ['Job opportunities', 'Quality of life', 'Education system', 'Climate and lifestyle'],
          links: [
            { text: 'Country Comparison Tool', url: '/ai-tools/country-comparison' },
    ...
    if (detectedIntents.includes('documents')) {
      return {
        id: Date.now().toString(),
        type: 'bot',
        content: "Document preparation is crucial! Here's what you'll typically need:\n\n📜 **Educational Documents**\n- Degree certificates\n- Transcripts\n- Credential assessments\n\n🗣️ **Language Proficiency**\n- IELTS/CELPIP/TEF results\n\n💼 **Work Experience**\n- Employment letters\n- Job descriptions\n- Reference letters\n\n💰 **Financial Documents**\n- Bank statements\n- Proof of funds\n\nWhich category would you like to explore first?",
        timestamp: new Date(),
        metadata: {
          suggestions: ['Educational documents', 'Language tests', 'Work experience proof', 'Financial requirements'],
          links: [
    ...
    return {
      ...
      timestamp: new Date(),
      metadata: {
        suggestions: ['Get personalized assessment', 'Explore country options', 'Check document requirements', 'Book expert consultation'],
        quickActions: [
