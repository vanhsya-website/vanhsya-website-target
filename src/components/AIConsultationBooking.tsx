  ...
  useEffect(() => {
    if (formData.destination && formData.visaType && formData.country) {
      const timeout = setTimeout(() => {
        setAiAnalysis(`Based on your profile (${formData.country} → ${formData.destination}, ${formData.visaType}), our AI recommends focusing on documentation requirements and timeline optimization. Success probability: 87%`);
      }, 1000);
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className={`text-xl ${VanhsyaTheme.text.secondary} max-w-4xl mx-auto mb-8`}>
            Get personalized immigration advice from our AI-enhanced expert consultants. 
            Book your consultation and receive instant AI analysis of your case.
          </p>
        </motion.div>
              ...
              <React.Fragment key={step.id}>
                ...
                >
                  ...
                  <span className={`text-sm font-medium ${
                    currentStep >= step.id ? VanhsyaTheme.text.primary : VanhsyaTheme.text.secondary
                  }`}>
                    {step.title}
                  </span>
                </motion.div>
        ...
        <div className="max-w-4xl mx-auto">
          <AnimatePresence mode="wait">
            ...
            {currentStep === 1 && (
              ...
              >
                <h2 className={`text-3xl font-bold ${VanhsyaTheme.text.primary} mb-8`}>
                  Personal Information
                </h2>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className={`block text-sm font-medium ${VanhsyaTheme.text.primary} mb-2`}>
                      Full Name *
                    </label>
                    <input
                      type="text"
                ...
                <div className="flex justify-end mt-8">
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    onClick={handleNext}
                    disabled={!formData.fullName || !formData.email || !formData.phone || !formData.country}
                ...
                <div className="mb-6">
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    {[
                      ...
                      { value: 'urgent', label: 'Urgent', desc: 'Need faster processing' },
                      { value: 'emergency', label: 'Emergency', desc: 'Immediate attention' }
                    ].map((option) => (
                      <motion.button
                        key={option.value}
                  ...
                  <div className="space-y-6">
                    ...
                    >
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        <h3 className={`text-xl font-bold ${VanhsyaTheme.text.primary}`}>
                          AI Recommendation
                        </h3>
                      </div>
                      {aiAnalysis ? (
                        <p className={VanhsyaTheme.text.secondary}>{aiAnalysis}</p>
                      ) : (
                    ...
                    >
                      <div className={`${VanhsyaTheme.backgrounds.glass} rounded-xl p-4 border border-white/20`}>
                        <div className="flex items-center gap-2 mb-2">
                          <Target className="w-5 h-5 text-green-400" />
                          <span className={`font-semibold ${VanhsyaTheme.text.primary}`}>
                            Success Rate
                          </span>
                        </div>
                      ...
                      <div className={`${VanhsyaTheme.backgrounds.glass} rounded-xl p-4 border border-white/20`}>
                        ...
                        <div className="text-2xl font-bold text-blue-400">6-8 mo</div>
                      </div>
                    </motion.div>
                  </div>
                    ...
                    <div className="space-y-3">
                      {[
                        ...
                        >
                          ...
                          <span className={VanhsyaTheme.text.primary}>{step}</span>
                        </motion.div>
                      ))}
                    </div>
                  </div>
                    ...
                    >
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className={VanhsyaTheme.text.secondary}>{slot.time}</span>
                        </div>
                      </div>

                      <div className={`font-bold ${VanhsyaTheme.text.primary} mb-2`}>
                        {slot.consultant}
                      </div>
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          <span className={`text-2xl font-bold ${VanhsyaTheme.text.primary}`}>
                            ${slot.price}
                          </span>
                          <span className={`text-sm ${VanhsyaTheme.text.secondary} ml-1`}>
                            / {slot.duration}min
                          </span>
                        </div>
                        {!slot.available && (
            ...
            {currentStep === 5 && (
              ...
              >
                <motion.div
                  ...
                  className="mb-8"
                >
                  <div className={`w-24 h-24 mx-auto bg-gradient-to-r ${VanhsyaTheme.gradients.primary} rounded-full flex items-center justify-center mb-6`}>
                    <CheckCircle className="w-12 h-12 text-white" />
                  </div>
                  <h2 className={`text-4xl font-bold ${VanhsyaTheme.text.primary} mb-4`}>
                    Booking Confirmed!
                  </h2>
                ...
                {selectedSlot && (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-left">
                      <div>
                        <span className={`${VanhsyaTheme.text.secondary} block`}>Consultant</span>
                        <span className={`font-semibold ${VanhsyaTheme.text.primary}`}>
                          {selectedSlot.consultant}
                        </span>
                      </div>
                      <div>
                        <span className={`${VanhsyaTheme.text.secondary} block`}>Date & Time</span>
                        <span className={`font-semibold ${VanhsyaTheme.text.primary}`}>
                          {new Date(selectedSlot.date).toLocaleDateString()} at {selectedSlot.time}
                        </span>
                      </div>
                      <div>
                        <span className={`${VanhsyaTheme.text.secondary} block`}>Duration</span>
                        <span className={`font-semibold ${VanhsyaTheme.text.primary}`}>
                          {selectedSlot.duration} minutes
                        </span>
                      </div>
                      <div>
                        <span className={`${VanhsyaTheme.text.secondary} block`}>Total Cost</span>
                        <span className={`font-semibold ${VanhsyaTheme.text.primary}`}>
                          ${selectedSlot.price}
                        </span>
                      </div>
