...
const knownScams: ScamAlert[] = [
  ...
  },
  {
    ...
    description:
      'Claims to be immigration officer requesting personal information',
    riskLevel: 'high',
...
export default function ScamDetector() {
  ...
  const [searchType, setSearchType] = useState<
    'all' | 'company' | 'website' | 'email' | 'phone'
  >('all');
  const [analysisInput, setAnalysisInput] = useState('');
  const [analysisType, setAnalysisType] = useState<
    'website' | 'email' | 'phone' | 'company'
  >('website');
  const [isAnalyzing, setIsAnalyzing] = useState(false);
  const [analysisResult, setAnalysisResult] = useState<AnalysisResult | null>(
    null
  );
  const [activeTab, setActiveTab] = useState<'search' | 'analyze' | 'report'>(
    'search'
  );
  ...
  const filteredScams = searchQuery
    ...
    : knownScams.filter(
        scam => searchType === 'all' || scam.type === searchType
      );
  ...
  const analyzeInput = async () => {
    ...
    setTimeout(() => {
      ...
      const isKnownScam = knownScams.some(
        scam =>
        ...
        details: {
          domainAge:
            analysisType === 'website'
              ? getDeterministicRandom('domain-age-' + input).next() > 0.5
                ? '2+ years'
                : 'Less than 6 months'
              : undefined,
          sslCertificate:
            analysisType === 'website'
              ? getDeterministicRandom('ssl-cert-' + input).next() > 0.3
              : undefined,
          contactInfo:
            getDeterministicRandom('contact-info-' + input).next() > 0.4,
          socialPresence:
            getDeterministicRandom('social-presence-' + input).next() > 0.3,
          reviews: Math.floor(
            getDeterministicRandom('reviews-' + input).next() * 100
          ),
          complaints: Math.floor(
            getDeterministicRandom('complaints-' + input).next() * 20
          ),
        },
      });
        ...
        <div className='container-max'>
          ...
          >
            ...
            <h1 className='heading-xl text-white mb-6'>
              Immigration{' '}
              <span className='text-gradient-cyan'>Scam Detector</span>
            </h1>
            <p className='text-xl text-white/90 max-w-3xl mx-auto leading-relaxed'>
              Protect yourself from immigration fraud. Check companies,
              websites, and communications against our database of known scams
              and get real-time risk assessments.
            </p>
          </motion.div>
        </div>
      </section>
                    ...
                    >
                      <div className='flex items-start justify-between mb-4'>
                        ...
                        <div className='text-right'>
                          <span
                            className={`px-3 py-1 rounded-full text-sm font-medium ${getRiskColor(scam.riskLevel)} ${getRiskBg(scam.riskLevel)}`}
                          >
                  ...
                  {analysisResult ? (
                    <div>
                      ...
                      <div className='mb-6'>
                        <div className='flex items-center justify-between mb-2'>
                          ...
                          <span
                            className={`font-bold ${getRiskColor(analysisResult.riskLevel)}`}
                          >
                        ...
                        <div className='mt-2'>
                          <span
                            className={`px-3 py-1 rounded-full text-sm font-medium ${getRiskColor(analysisResult.riskLevel)} ${getRiskBg(analysisResult.riskLevel)}`}
                          >
                      ...
                      <div className='bg-gray-50 rounded-lg p-4'>
                        ...
                        <div className='grid grid-cols-2 gap-4 text-sm'>
                          ...
                          {analysisResult.details.sslCertificate !==
                            undefined && (
                            <div>
                              ...
                              <span
                                className={`ml-2 font-medium ${analysisResult.details.sslCertificate ? 'text-green-600' : 'text-red-600'}`}
                              >
                                {analysisResult.details.sslCertificate
                                  ? 'Valid'
                                  : 'Invalid'}
                          ...
                          <div>
                            ...
                            <span
                              className={`ml-2 font-medium ${analysisResult.details.contactInfo ? 'text-green-600' : 'text-red-600'}`}
                            >
                              {analysisResult.details.contactInfo
                                ? 'Available'
                                : 'Missing'}
                            </span>
                          </div>
                          <div>
                            ...
                            <span className='ml-2 font-medium'>
                              {analysisResult.details.reviews}
                            </span>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className='text-center py-12'>
                      ...
                      <p className='text-gray-500'>
                        Enter an entity to analyze and click "Analyze Risk".
                      </p>
                    </div>
                  )}
                </div>
