...
import { Shield, AlertTriangle, CheckCircle, XCircle, Search, Upload, Link, Mail, Phone, ArrowLeft, Home } from 'lucide-react';
  ...
  const analyzeInput = async () => {
    ...
    setTimeout(() => {
      ...
      setAnalysisResult({
        ...
        recommendations,
        details: {
          domainAge: analysisType === 'website' ? getDeterministicRandom('domain-age-' + input).next() > 0.5 ? '2+ years' : 'Less than 6 months' : undefined,
          sslCertificate: analysisType === 'website' ? getDeterministicRandom('ssl-cert-' + input).next() > 0.3 : undefined,
          contactInfo: getDeterministicRandom('contact-info-' + input).next() > 0.4,
          socialPresence: getDeterministicRandom('social-presence-' + input).next() > 0.3,
          reviews: Math.floor(getDeterministicRandom('reviews-' + input).next() * 100),
          complaints: Math.floor(getDeterministicRandom('complaints-' + input).next() * 20)
        }
      });
