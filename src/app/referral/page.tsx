...
const tierBenefits = {
  Bronze: { 
    ...
    benefits: ['Basic commission structure', 'Access to referral dashboard', 'Monthly rewards']
  },
  Silver: { 
    commission: 'Enhanced Commission', 
    minReferrals: 5, 
    color: 'from-gray-400 to-gray-600',
    benefits: ['Higher commission rates', 'Priority support', 'Quarterly bonuses']
  },
  Gold: { 
    commission: 'Premium Commission', 
    minReferrals: 15, 
    color: 'from-yellow-400 to-yellow-600',
    benefits: ['Premium commission structure', 'VIP support line', 'Annual rewards']
  },
  Platinum: { 
    commission: 'Elite Commission', 
    minReferrals: 30, 
    color: 'from-purple-400 to-purple-600',
    benefits: ['Elite commission rates', 'Dedicated account manager', 'Exclusive events']
  },
  Diamond: { 
    commission: 'VIP Commission', 
    minReferrals: 50, 
    color: 'from-blue-400 to-cyan-400',
    benefits: ['Maximum commission structure', 'Personal consultant', 'Luxury rewards']
  }
};
        ...
        <div className="mx-auto text-center max-w-7xl">
          ...
          >
            <div className="inline-flex items-center gap-3 px-6 py-3 mb-6 border rounded-full bg-gradient-to-r from-green-400/20 to-blue-500/20 border-green-400/30">
              ...
              <span className="font-medium text-green-300">VANHSYA Partner Program</span>
            </div>
            
            <h1 className="mb-6 text-4xl font-bold text-white md:text-6xl">
              Join Our Elite
              <span className="text-transparent bg-gradient-to-r from-green-400 via-blue-400 to-purple-400 bg-clip-text"> Partner Network</span>
            </h1>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {referralPrograms.map((program, index) => (
              ...
              >
                <h3 className="mb-3 text-xl font-bold text-white">{program.service}</h3>
                <p className="mb-4 text-sm text-gray-300">{program.description}</p>
                
                <div className="space-y-3">
                  <div className="flex items-center justify-between p-3 border bg-green-500/20 rounded-xl border-green-500/30">
                    <span className="font-medium text-green-400">Success Guarantee</span>
                    <span className="text-sm text-green-300">{program.guarantee}</span>
                  </div>
                  
                  <div className="flex items-center justify-between p-3 border bg-blue-500/20 rounded-xl border-blue-500/30">
                    <span className="font-medium text-blue-400">Partner Rewards</span>
                    <span className="text-sm text-blue-300">{program.commission}</span>
                  </div>
                </div>
        ...
        <div className="max-w-4xl mx-auto">
          <div className="p-8 border bg-white/10 backdrop-blur-sm rounded-2xl border-white/20">
            <div className="mb-8 text-center">
              <h2 className="mb-2 text-3xl font-bold text-white">Your Partner Dashboard</h2>
              <p className="text-gray-400">Track your progress and earnings</p>
            </div>
            
            <div className="grid gap-6 mb-8 md:grid-cols-3">
              <div className="text-center">
                <div className="mb-1 text-3xl font-bold text-blue-400">{userStats.totalReferrals}</div>
                <div className="text-sm text-gray-400">Total Referrals</div>
              </div>
              <div className="text-center">
                <div className="mb-1 text-3xl font-bold text-green-400">{userStats.successfulReferrals}</div>
                <div className="text-sm text-gray-400">Successful Cases</div>
              </div>
              <div className="text-center">
                <div className="mb-1 text-3xl font-bold text-purple-400">{formatPrice(userStats.totalEarnings)}</div>
                <div className="text-sm text-gray-400">Total Earnings</div>
              </div>
            </div>

            <div className="p-6 mb-6 bg-black/30 rounded-xl">
              <h3 className="mb-4 text-lg font-bold text-white">Your Partner Link</h3>
          ...
          <div className="grid gap-4 md:grid-cols-3 lg:grid-cols-5">
            {Object.entries(tierBenefits).map(([tier, benefits]) => (
              ...
              >
                ...
                <p className="mb-4 text-sm text-gray-400">{benefits.minReferrals}+ referrals</p>
                
                <div className="space-y-2">
                  <div className="text-sm font-medium text-blue-400">{benefits.commission}</div>
                  {benefits.benefits.map((benefit, idx) => (
                    <div key={idx} className="text-xs text-gray-300">{benefit}</div>
                  ))}
                </div>
