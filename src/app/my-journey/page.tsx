...
import { 
  CheckCircle, Star, Award, Trophy, Target,
...
export default function VanhsyaJourney() {
  ...
  const [activeSection, setActiveSection] = useState<'overview' | 'achievements' | 'referrals' | 'rewards'>('overview');
  ...
  const getTierColor = (tier: string) => {
    switch (tier) {
      case 'bronze': return 'from-orange-500 to-yellow-500';
        ...
        <div className="max-w-7xl mx-auto text-center">
          ...
          >
            <div className="inline-flex items-center gap-3 px-6 py-3 bg-gradient-to-r from-purple-400/20 to-pink-500/20 rounded-full border border-purple-400/30 mb-6">
              ...
              <span className="text-purple-300 font-medium">Your Migration Journey</span>
            </div>
            
            <h1 className="text-4xl md:text-6xl font-bold text-white mb-6">
              Welcome to Your
              <span className="bg-gradient-to-r from-purple-400 via-pink-400 to-blue-400 bg-clip-text text-transparent"> VANHSYA Journey</span>
            </h1>
            
            <p className="text-xl text-gray-300 max-w-3xl mx-auto mb-8">
              Track your progress, celebrate achievements, and earn rewards as you navigate your migration journey with the world's most advanced AI-powered platform.
            </p>
            ...
            <div className="max-w-4xl mx-auto bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20">
              <div className="grid md:grid-cols-4 gap-6">
                <div className="text-center">
                  <div className="text-3xl font-bold text-green-400 mb-1">{formatPrice(userStats.totalEarnings)}</div>
                  <div className="text-sm text-gray-400">Total Earnings</div>
                </div>
                <div className="text-center">
                  <div className="text-3xl font-bold text-blue-400 mb-1">{userStats.referralCount}</div>
                  <div className="text-sm text-gray-400">Successful Referrals</div>
                </div>
                <div className="text-center">
                  <div className="text-3xl font-bold text-purple-400 mb-1">{userStats.completedSteps}/{userStats.totalSteps}</div>
                  <div className="text-sm text-gray-400">Journey Progress</div>
                </div>
                <div className="text-center">
                  <div className="text-3xl font-bold text-pink-400 mb-1">{userStats.achievementsUnlocked}</div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-wrap justify-center gap-2 mb-8">
            {[
              ...
              <button
                key={tab.id}
                onClick={() => setActiveSection(tab.id as 'overview' | 'achievements' | 'referrals' | 'rewards')}
                ...
                {journeySteps.map((step, index) => (
                  ...
                  >
                    ...
                    <div className={`relative z-10 w-16 h-16 rounded-full flex items-center justify-center border-4 ${
                      ...
                      step.status === 'current' ? 'bg-blue-500 border-blue-400' :
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        {step.reward && (
                          <div className="flex items-center gap-1 px-3 py-1 bg-green-500/20 rounded-full">
                            ...
                            <span className="text-green-400 font-medium">+{formatPrice(step.reward)}</span>
                          </div>
                        )}
                      </div>
                    </div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {achievements.map((achievement) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between">
                    <div className={`px-3 py-1 rounded-full text-xs font-medium bg-gradient-to-r ${getTierColor(achievement.tier)}`}>
                      {achievement.tier.toUpperCase()}
                    </div>
                    <div className="flex items-center gap-1">
                      <Award className="w-4 h-4 text-yellow-400" />
                      <span className="text-yellow-400 font-medium">+{formatPrice(achievement.reward)}</span>
                    </div>
                  </div>
                </motion.div>
              ))}
      ...
      <section className="py-12 px-6">
        <div className="max-w-4xl mx-auto">
          ...
          <SocialShare 
            ...
            hashtags={['VANHSYASuccess', 'Migration', 'Dreams', 'AI', 'Success']}
